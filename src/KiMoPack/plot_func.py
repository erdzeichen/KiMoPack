...
if 1:  # Hide imports
    ...
    try:
        ...
        print(
            "the h5py module was not imported that allows to save the projects files on windows you can install that with pip install h5py"
        )
    try:
        import keyboard
    except:
        print(
            'the keyboard module was not imported. on Windows this allows to stop the fit by pressing q/n you can install it with "pip install keyboard" '
        )
    ...
    pandas.options.mode.chained_assignment = (
        None  # I use this a lot and think I can ignore it
    )
    FWHM = 2.35482
...
print(
    ...
    % (version, os.path.dirname(os.path.realpath(__file__)))
)
...
def download_notebooks():
    ...
    for f in list_of_tools:
        url = (
            "https://raw.githubusercontent.com/erdzeichen/KiMoPack/main/Workflow_tools/%s"
            % f
        )
        print("Downloading Workflow Tools/%s" % f)
        with open(
            check_folder(path="Workflow_tools", current_path=os.getcwd(), filename=f),
            "wb",
        ) as out:
...
def download_all():
    ...
    for f in list_of_example_data:
        url = (
            "https://raw.githubusercontent.com/erdzeichen/KiMoPack/main/Workflow_tools/Data/%s"
            % f
        )
        print("Downloading Workflow Tools/Data/%s" % f)
        with open(
            check_folder(
                path="Workflow_tools" + os.sep + "Data",
    ...
    for f in list_of_tutorials:
        url = (
            "https://raw.githubusercontent.com/erdzeichen/KiMoPack/main/Tutorial_Notebooks/%s"
            % f
        )
        print("Downloading tutorial %s" % f)
        with open(
            check_folder(
                path="Tutorial_Notebooks", current_path=os.getcwd(), filename=f
            ),
            "wb",
        ) as out:
            r = http.request("GET", url, preload_content=False)
    ...
    for key in tutorial_data.keys():
        for f in tutorial_data[key]:
            if "Master" in key:
                url = (
                    "https://raw.githubusercontent.com/erdzeichen/KiMoPack/main/Tutorial_Notebooks/Data/Compare/Master/%s"
                    % f
                )
                with open(
                    check_folder(
                        path=os.sep.join(
                            ["Tutorial_Notebooks", "Data", "Compare", "Master"]
                        ),
                        current_path=os.getcwd(),
                        filename=f,
                    ),
                ...
                url = (
                    "https://raw.githubusercontent.com/erdzeichen/KiMoPack/main/Tutorial_Notebooks/Data/key/%s"
                    % f
                )
                with open(
                    check_folder(
                        path=os.sep.join(["Tutorial_Notebooks", "Data", key]),
                        current_path=os.getcwd(),
                        filename=f,
                    ),
                    "wb",
    ...
    for f in tutorial_images:
        url = (
            "https://raw.githubusercontent.com/erdzeichen/KiMoPack/main/Tutorial_Notebooks/img/%s"
            % f
        )
        with open(
            check_folder(
                path=os.sep.join(["Tutorial_Notebooks", "img"]),
                current_path=os.getcwd(),
                filename=f,
            ),
            "wb",
...
def changefonts(
    weight="bold", font="standard", SMALL_SIZE=11, MEDIUM_SIZE=13, LARGE_SIZE=18
):
    """
    ...
    plt.rc(
        "axes", titlesize=LARGE_SIZE, labelweight=weight
    )  # fontsize of the axes title
    plt.rc(
        "axes", labelsize=LARGE_SIZE, labelweight=weight
...
def GUI_open(
    ...
    sep="\t",
    decimal=".",
    index_is_energy=False,
    transpose=False,
    sort_indexes=False,
    divide_times_by=None,
    shift_times_by=None,
    external_time=None,
    external_wave=None,
    use_same_name=True,
    ...
    if project_list is None:
        ...
        path_list = filedialog.askopenfilename(
            ...
            multiple=True,
            filetypes=[("TA project files", "*.%s" % fileending)],
        )
        if project_list is None:
            project_list = []
        ...
        if (
            filename_part is not None
        ):  # we specified a specific name and want only the files with this name in it
            path_list = sorted(
                [
                    os.path.join(scan_path, name)
                    for name in os.listdir(scan_path)
                    if name.endswith(fileending) and filename_part in name
                ]
            )
        else:  # we have not specified a specific name and want all files in the folder
            path_list = sorted(
                [currentFile for currentFile in scan_path.glob("*.%s" % fileending)]
            )
    else:
        if len(project_list) < 1:
            raise ValueError(
                "The use_gui switch is ment to bypass the gui, but you still need at least some files as a list"
            )
        else:
            if isinstance(project_list, str):
                project_list = [project_list]
            if not hasattr(project_list, "__iter__"):
                project_list = [project_list]
            path_list = []
            for filename in project_list:
                ta = check_folder(
                    path=path, filename=filename, current_path=os.getcwd()
                )
                path_list.append(ta)
    return_list = []
    for entrance in path_list:
        try:
            ...
            ta = TA(
                ...
                sep=sep,
                decimal=decimal,
                index_is_energy=index_is_energy,
                transpose=transpose,
                sort_indexes=sort_indexes,
                divide_times_by=divide_times_by,
                shift_times_by=shift_times_by,
    ...
    if isinstance(ori_df, pandas.DataFrame):
        ...
        for col in ori_df.columns:
            new_df[col] = np.interp(
                new_x, ori_df.index.values.astype("float"), ori_df[col].values
            )
...
def savitzky_golay(y, window_size, order, deriv=0, rate=1):
    ...
    return savgol_filter(
        x=y, window_length=window_size, polyorder=order, deriv=deriv, delta=rate
    )
...
def Summarize_scans(
    ...
    window1=None,
    window2=None,
    save_name="combined.SIA",
    fileending="SIA",
    filename_part="Scan",
    return_removed_list=False,
    sep="\t",
    decimal=".",
    index_is_energy=False,
    transpose=False,
    ...
    base_TA_object=None,
    value_filter=None,
    zscore_filter_level=None,
    zscore_in_window=True,
    dump_times=True,
    replace_values=None,
    drop_scans=False,
):
    ...
    if list_of_scans is None:
        ...
        path_list = filedialog.askopenfilename(
            initialdir=os.getcwd(),
            multiple=True,
            filetypes=[("Raw scan files", "*.%s" % fileending)],
        )
    ...
    if not isinstance(
        list_of_scans[0], TA
    ):  # we do not have opened file but most likely a list of names
        try:
            list_of_projects = []
            for entrance in list_of_scans:
                ...
                if base_TA_object is None:
                    ...
                    try:
                        new_ds = sub_ds(
                            new_ds,
                            ignore_time_region=base_TA_object.ignore_time_region,
                            wave_nm_bin=base_TA_object.wave_nm_bin,
                            baseunit=base_TA_object.baseunit,
                            scattercut=base_TA_object.scattercut,
                            bordercut=base_TA_object.bordercut,
                            timelimits=base_TA_object.timelimits,
                            time_bin=base_TA_object.time_bin,
                            equal_energy_bin=base_TA_object.equal_energy_bin,
                        )
                        if (base_TA_object.wave_nm_bin is not None) or (
                            base_TA_object.equal_energy_bin is not None
                        ):
                            print(
                                'in the original TA objec the data was rebinned, which is now also done for the single scans. To avoid that use "ta.wave_nm_bin = None" and / or "ta.equal_energy_bin = None" before handing it to base_TA_object'
                            )
                    except:
            ...
            try:
                ...
                print(
                    "the stacking of the scans failed, are you sure that all are have the same shape"
                )
            #######################
        except:
            raise ValueError(
                "Sorry did not understand the project_list entry, use GUI_open to create one"
            )
    else:
        try:
            list_of_projects = []
    ...
    if window1 is None:
        window1 = [
            ds.index.values.min(),
            ds.index.values.max(),
            ds.columns.values.min(),
            ds.columns.values.max(),
        ]
    ...
    if (zscore_filter_level is not None) or (value_filter is not None):
        ...
        if value_filter is not None:
            ...
            outside_range = np.invert(
                log_and(dataset > lowervalue, dataset < uppervalue)
            )
            if dump_times:  # this is default
                outside_range = np.tile(
                    outside_range.all(axis=1, keepdims=True), (1, dataset.shape[1], 1)
                )
            elif drop_scans:
                outside_range = np.tile(
                    outside_range.any(axis=1, keepdims=True), (1, dataset.shape[1], 1)
                )
                outside_range = np.tile(
                    outside_range.any(axis=0, keepdims=True), (dataset.shape[0], 1, 1)
                )
            dataset[outside_range] = replace_values
        if zscore_filter_level is not None:
            if zscore_in_window:
                window1_index = [
                    find_nearest_index(ds.index.values, window1[0]),
                    find_nearest_index(ds.index.values, window1[1]),
                    find_nearest_index(ds.columns.values, window1[2]),
                    find_nearest_index(ds.columns.values, window1[3]),
                ]
                vector = np.nanmean(
                    np.nanmean(
                        dataset[
                            window1_index[0] : window1_index[1],
                            window1_index[2] : window1_index[3],
                            :,
                        ],
                        axis=0,
                    ),
                ...
                good = log_and(
                    vector
                    > (np.nanmean(vector) - zscore_filter_level * np.nanstd(vector)),
                    vector
                    < (np.nanmean(vector) + zscore_filter_level * np.nanstd(vector)),
                )
                if not window2 is None:
                    window2_index = [
                        find_nearest_index(ds.index.values, window2[0]),
                        find_nearest_index(ds.index.values, window2[1]),
                        find_nearest_index(ds.columns.values, window2[2]),
                        find_nearest_index(ds.columns.values, window2[3]),
                    ]
                    vector = np.nanmean(
                        np.nanmean(
                            dataset[
                                window2_index[0] : window2_index[1],
                                window2_index[2] : window2_index[3],
                                :,
                            ],
                            axis=0,
                        ),
                ...
                if drop_scans:
                    outside_range = np.tile(
                        outside_range.any(axis=1, keepdims=True),
                        (1, dataset.shape[1], 1),
                    )
                    outside_range = np.tile(
                        outside_range.any(axis=0, keepdims=True),
                        (dataset.shape[0], 1, 1),
                    )
                elif dump_times:
                    outside_range = np.tile(
    ...
    #############manual filtering################
        ...
        if baseunit is None:
            baseunit = ds.index.name
        if units is None:
            units = ds.columns.name
        if list_to_dump is not None:
            if list_to_dump == "single":
                ...
                if not hasattr(
                    list_to_dump, "__iter__"
                ):  # we have only a single number/name in there
                    list_to_dump = [list_to_dump]
                filenames_to_dump = []
                for entry in list_to_dump:
                    try:
                        filenames_to_dump.append(
                            list_of_scans[entry].filename
                        )  # list_of_scans is a list of TA objects that have filename and if entry is an index of this list this goes well
                    except:
                        filenames_to_dump.append(
                            entry
                        )  # we assume it is already a filename
                list_to_dump = []
            ...
            for i in range(30):  # we make a maximum of 30 rounds
                ...
                series1.name = "%.3g:%.3g %s at %.1f:%.1f %s" % (
                    window1[0],
                    window1[1],
                    baseunit,
                    window1[2],
                ...
                if not window2 is None:
                    ...
                    series2.name = "%.3g:%.3g %s at %.1f:%.1f %s" % (
                        window2[0],
                        window2[1],
                        baseunit,
                        window2[2],
                    ...
                    if len(series1) > 15:
                        ...
                        ax2.fill_between(
                            x=range(len(series1)),
                            y1=gol - 2 * series1.var(),
                            y2=gol + 2 * series1.var(),
                            color=colm(4),
                            alpha=0.3,
                        )
                else:
                    ...
                    if len(series1) > 15:
                        ...
                        ax.fill_between(
                            x=range(len(series1)),
                            y1=gol - 2 * np.nanvar(series1.values),
                            y2=gol + 2 * np.nanvar(series1.values),
                            color=colm(2),
                            alpha=0.3,
                        )
                        # except:
                ...
                if list_to_dump == "single":
                    ...
                    ax.set_title(
                        "click on the first and last scan to be removed, repeat as long as necessary\n an empty middle click ends the process"
                    )
                    polypts = np.asarray(
                        plt.ginput(
                            n=int(len(series1) / 2),
    ...
    try:
        ds = pandas.DataFrame(
            np.nanmean(list_of_projects, axis=2), index=ds.index, columns=ds.columns
        )
    except:
        print("nanmean failed, assume single wavelength")
        ds = pandas.DataFrame(
            np.mean(list_of_projects), index=ds.index, columns=ds.columns
        )
...
def sub_ds(
    ds,
    times=None,
    time_width_percent=0,
    ignore_time_region=None,
    drop_ignore=False,
    wave_nm_bin=None,
    baseunit=None,
    scattercut=None,
    ...
    if (equal_energy_bin is not None) and (
        wavelength is None
    ):  # we work with optical data but want to bin in equal energy
        ...
        x = (
            scipy.constants.h
            * scipy.constants.c
            / (x * 1e-9 * scipy.constants.electron_volt)
        )
        if from_fit:  # they are already binned
            ...
            rebin_max = np.argmin(
                (x[1:] - x[:-1]) < equal_energy_bin
            )  # find the position where the difference is larger than the wave_nm_bin
            if rebin_max == 0:
                rebin_max = len(x)  # we get 0 when all teh values are ok
            if rebin_max < len(x):
                if (x[1:] - x[:-1] > equal_energy_bin).all():
                    raise ValueError("equal_energy_bin bins are to small for the data")
                bins = np.arange(x.min(), x[rebin_max], equal_energy_bin)
                bin_means, bin_edges = binned_statistic(
                    x[:rebin_max], ds.values[:, :rebin_max], statistic="mean", bins=bins
                )[:2]
                bins = (bin_edges[1:] + bin_edges[:-1]) / 2.0
                ds = pandas.concat(
                    (
                        pandas.DataFrame(bin_means, index=y, columns=bins),
                        ds.iloc[:, rebin_max:],
                    ),
                    axis=1,
                    join="outer",
                ...
                bin_means, bins = binned_statistic(
                    x, ds.values, statistic="mean", bins=bins
                )[:2]
        ...
        x = ds.columns.values.astype("float")
        y = ds.index.values.astype("float")
        if (x[1:] - x[:-1] > wave_nm_bin).all():
            raise ValueError("wavelength_nm_bins bins are to small for the data")
        rebin_max = np.argmin(
            (x[1:] - x[:-1]) < wave_nm_bin
        )  # find the position where the difference is larger than the wave_nm_bin
        if rebin_max == 0:
    ...
    if time_bin is not None:
        ...
        bin_means, bins = binned_statistic(
            time, ds.values.T, statistic="mean", bins=time_bins
        )[:2]
        bins = (bins[1:] + bins[:-1]) / 2.0
        ds = pandas.DataFrame(bin_means, index=y, columns=bins)
        ds = ds.T
        ...
        if isinstance(scattercut[0], numbers.Number):
            if equal_energy_bin is not None:
                scattercut = [
                    scipy.constants.h
                    * scipy.constants.c
                    / (a * 1e-9 * scipy.constants.electron_volt)
                    for a in scattercut
                ]
        ...
        for i, wave in enumerate(wavelength):
            ...
            if equal_energy_bin is not None and from_fit:
                upper = (
                    scipy.constants.h
                    * scipy.constants.c
                    / (lower * 1e-9 * scipy.constants.electron_volt)
                )
                lower = (
                    scipy.constants.h
                    * scipy.constants.c
                    / (upper * 1e-9 * scipy.constants.electron_volt)
                )
                wave = (
                    scipy.constants.h
                    * scipy.constants.c
                    / (wave * 1e-9 * scipy.constants.electron_volt)
                )
            if i == 0:
        ...
        if time_width_percent > 0:
            for i, time in enumerate(times):
                if time < 0:
                    limits = [
                        find_nearest_index(
                            time_scale, time + time * time_width_percent / 100.0
                        ),
                        find_nearest_index(
                            time_scale, time - time * time_width_percent / 100.0
                        ),
                ...
                if i == 0:
                    ...
                    out.columns = [
                        "%.3g %s (%.3g - %.3g %s)"
                        % (time_mean, baseunit, time_lower, time_upper, baseunit)
                    ]
                else:
                    out[
                        "%.3g %s (%3g - %.3g %s)"
                        % (time_mean, baseunit, time_lower, time_upper, baseunit)
                    ] = (ds.iloc[limits[0] : limits[1], :].mean(axis="rows").to_frame())
        else:
            for i, time in enumerate(times):
...
def plot2d(
    ...
    if cmap is None:
        ...
        [
            isinstance(cmap, type(cm.viridis)),
            isinstance(cmap, type(cm.jet)),
            isinstance(cmap, type(cm.Blues)),
            isinstance(cmap, type(cm.coolwarm)),
            isinstance(cmap, type(cm.terrain)),
        ]
    ).any():  # we must have a
        cmap = standard_map
    ...
    ds = sub_ds(
        ds,
        scattercut=scattercut,
        bordercut=bordercut,
        timelimits=timelimits,
        wave_nm_bin=wave_nm_bin,
    ...
    if intensity_range is None:
        ...
        if not hasattr(intensity_range, "__iter__"):  # lets have an lazy option
            ...
            if log_scale:
                print(
                    "I highly recommend to make a symmetric intensity distribution for logarithmic scale, the colorbar might look strange otherwise"
                )
    if log_scale:
        if 0:  # old manual symlog
            bounds0 = list(
                ...
                * np.logspace(
                    ...
                    np.log10(-intensity_range[0] / (levels / 2)),
                    levels,
                )
            )
            bounds1 = np.logspace(
                np.log10(intensity_range[1] / (levels / 2)),
            ...
            norm = SymLogNorm(
                abs(max(intensity_range) - min(intensity_range)) / 100,
                linscale=1.0,
                vmin=min(intensity_range),
                vmax=max(intensity_range),
                clip=True,
                base=10,
            )
            mid_color = cmap(0.5)
        ...
        if 0:
            nbins = levels
            levels = LinearLocator(numticks=levels).tick_values(
                vmin=min(intensity_range), vmax=max(intensity_range)
            )
    ...
    if ignore_time_region is None:
        ...
        try:
            ...
            if equal_energy_bin is not None:
                rect = plt.Rectangle(
                    (x.max(), lower),
                    width=abs(ax.get_xlim()[0] - ax.get_xlim()[1]),
    ...
    if use_colorbar:
        ...
        if log_scale:
            values = [
                intensity_range[0],
                mid - abs(intensity_range[0] - mid) / 10,
                mid - abs(intensity_range[0] - mid) / 100,
                mid,
                mid + abs(intensity_range[1] - mid) / 100,
                mid + abs(intensity_range[1] - mid) / 10,
                intensity_range[1],
            ]
        else:
            values = [
                intensity_range[0],
                intensity_range[0] + abs(intensity_range[0] - mid) / 2,
                mid,
                intensity_range[1] - abs(intensity_range[1] - mid) / 2,
                intensity_range[1],
            ]
        ...
        fontsize -= 4
        if (
            not data_type is None
        ):  # we use this as a switch to enable a flexible avoidance of the label setting.
            if log_scale:
                if ax_ori:
                    cbar.set_label(
                        data_type + "\nLog-scale",
                ...
                if ax_ori:
                    cbar.set_label(
                        data_type, rotation=270, labelpad=20, fontsize=fontsize
                    )
                else:
                    cbar.set_label(
                        data_type, rotation=270, labelpad=20, fontsize=fontsize
    ...
    if "symlog" in plot_type:
        ...
        if 1:
            ...
            locmaj = matplotlib.ticker.LogLocator(
                base=10.0, subs=(0.1, 1.0, 10.0, 1e2, 1e3, 1e4)
            )
            ax.yaxis.set_major_locator(locmaj)
            locmin = matplotlib.ticker.LogLocator(
                base=10.0, subs=np.arange(0.1, 1, 0.1)
            )
            ...
            if timelimits[1] > 100:
                ...
                ticks = np.concatenate(
                    (
                        ticks.clip(min=0.1),
                        np.zeros(1),
                        ticks.clip(max=-0.1, min=timelimits[0]),
                    ),
                    axis=0,
                )
            ax.set_yticks(ticks)
            ...
            print("here2")
            ax.set_yscale(
                "symlog", linthresh=lintresh, subsy=range(2, 9), linscaley=lintresh
            )
...
def plot2d_fit(
    ...
    if patches:
        plot2d(
            ...
            ax=ax[0],
            baseunit=baseunit,
            use_colorbar=plot_with_colorbar,
            levels=levels,
            plot_type=scale_type,
            ignore_time_region=ignore_time_region,
            lintresh=lintresh,
            bordercut=bordercut,
            scattercut=scattercut,
        ...
        plot2d(
            ...
            cmap=cmap,
            log_scale=log_scale,
            intensity_range=np.array(intensity_range) / error_matrix_amplification,
            ax=ax[2],
            baseunit=baseunit,
            use_colorbar=plot_with_colorbar,
            levels=levels,
        ...
        for i in range(3):
            ...
            rect = plt.Rectangle(
                (ax[i].get_xlim()[1] - x_width, ax[i].get_ylim()[1] - y_width),
                x_width,
                y_width,
                facecolor="white",
                alpha=0.5,
            ...
            ax[i].add_patch(rect)
            ax[i].text(
                ax[i].get_xlim()[1] - x_width + x_width * 0.1,
                ax[i].get_ylim()[1] - y_width + y_width * 0.1,
                stringen[i],
                fontsize=16,
            )
            fig.subplots_adjust(
                left=0.15, bottom=0.067, right=0.97, top=0.985, wspace=0.0, hspace=0.258
            )
    else:
        plot2d(
            re["A"],
...
def plot_fit_output(
    ...
    filename=None,
    f="standard",
    intensity_range=1e-2,
    baseunit="ps",
    timelimits=None,
    scattercut=None,
    bordercut=None,
    error_matrix_amplification=20,
    wave_nm_bin=5,
    rel_wave=None,
    width=10,
    rel_time=[1, 5, 10],
    time_width_percent=10,
    ignore_time_region=None,
    save_figures_to_folder=True,
    log_fit=False,
    mod=None,
    subplot=False,
    color_offset=0,
    log_scale=True,
    savetype="png",
    evaluation_style=False,
    lintresh=1,
    scale_type="symlog",
    patches=False,
    print_click_position=False,
    data_type="differential Absorption in $\mathregular{\Delta OD}$",
	...
	*Contents of the plots*
    ...
    if mod is not None:
        if not isinstance(mod, type("hello")):
            mod = "ext. func."
    if evaluation_style:
        if mod is not None:
        ...
        try:
            stringen.append(
                timedf.to_string(
                    columns=["value", "init", "vary", "min", "max", "expr"],
            ...
            print(
                'something strange happened, most likely one value went "inf" or is set unexpectedly to None'
            )
    else:
        if mod is not None:
            if mod in ["paral", "exponential"]:
                stringen.append("Fit with ind.\nexpon. decays:")
            else:
                stringen.append("Fit with time parameters:")
        try:
        ...
        stringen.append(
            timedf.to_string(
                columns=["value"], float_format="{:.3g}".format, header=False
            )
        )
    stringen = "\n".join(stringen)
    times = timedf[timedf.is_rate].loc[:, "value"].values
    time_string = timedf[timedf.is_rate].to_string(
        columns=["value"], float_format="{:.3g}".format, header=False
    )
    if not hasattr(plotting, "__iter__"):
        plotting = [plotting]
    if 0 in plotting:  # DAC
        ...
        for i, col in enumerate(DAC_copy):
            DAC_copy.iloc[:, i] = (
                DAC_copy.iloc[:, i].values * re["c"].abs().max().iloc[i]
            )
        if scattercut is None:
            ...
            DAC.loc[: scattercut[0], :].plot(
                ax=ax1b, color=colm(range(n_colors), cmap=cmap)
            )
            DAC.loc[scattercut[1] :, :].plot(
                ax=ax1b, color=colm(range(n_colors), cmap=cmap), label="_nolegend_"
            )
            normed.loc[: scattercut[0], :].plot(
                ax=ax1a, color=colm(range(n_colors), cmap=cmap)
            )
            normed.loc[scattercut[1] :, :].plot(
                ax=ax1a, color=colm(range(n_colors), cmap=cmap), label="_nolegend_"
            )
            DAC_copy.loc[: scattercut[0], :].plot(
                ax=ax1c, color=colm(range(n_colors), cmap=cmap)
            )
            DAC_copy.loc[scattercut[1] :, :].plot(
                ax=ax1c, color=colm(range(n_colors), cmap=cmap), label="_nolegend_"
            )
        else:
            try:
                scattercut = flatten(scattercut)
                for i in range(len(scattercut) / 2 + 1):
                    if i == 0:
                        ...
                        DAC.loc[scattercut[2 * i - 1] : scattercut[2 * i], :].plot(
                            ax=ax1b,
                            color=colm(range(n_colors), cmap=cmap),
                            label="_nolegend_",
                        )
                        normed.loc[scattercut[2 * i - 1] : scattercut[2 * i], :].plot(
                            ax=ax1a,
                            color=colm(range(n_colors), cmap=cmap),
                            label="_nolegend_",
                        )
                        DAC_copy.loc[scattercut[2 * i - 1] : scattercut[2 * i], :].plot(
                            ax=ax1c,
                            color=colm(range(n_colors), cmap=cmap),
                            label="_nolegend_",
                        )
        ...
        if mod in ["paral", "exponential"]:
            try:
                names = [
                    "decay %i: %.3g %s" % (i, a, baseunit) for i, a in enumerate(times)
                ]
            except:
                print(
                    'something strange happened, most likely one value went "inf" or is set unexpectedly to None'
    ...
    if 1 in plotting:  # -------plot sum_sum------------
        ...
        dat = [
            pandas.DataFrame(re["A"], index=re["A"].index, columns=re["A"].columns)
            .abs()
            .sum(axis=1)
        ]
        dat.append(
            pandas.DataFrame(re["AC"], index=re["AC"].index, columns=re["AC"].columns)
            .abs()
            .sum(axis=1)
        )
        dat.append(
            pandas.DataFrame(re["AE"], index=re["AE"].index, columns=re["AE"].columns)
            .abs()
            .sum(axis=1)
        )
                        ...
                        for i in range(3):
                            for j in range(3):
                                if i == 2:
                                    _ = (
                                        dat[i]
                                        .loc[: ignore_time_region_loc[k]]
                                        .plot(
                                            ax=ax2a[j],
                                            label=dat_names[i],
                                            style=dat_styles[i],
                                            color=colm(i, cmap=cmap),
                                        )
                                    )
                                else:
                                    _ = (
                            ...
                            for j in range(3):
                                if i == 2:
                                    _ = (
                                        dat[i]
                                        .loc[
                                            ignore_time_region_loc[
                                                2 * k - 1
                                            ] : ignore_time_region_loc[2 * k]
                                        ]
                                        .plot(
                                            ax=ax2a[j],
                                            label="_nolegend_",
    ...
    if 2 in plotting:  # ---plot single wavelength----------
        ...
        for t in times:
            if isinstance(t, float):
                ax3.plot(
                    [t, t], ax3.get_ylim(), lw=0.5, zorder=10, alpha=0.5, color="black"
                )
        fig3.tight_layout()
    if 3 in plotting:  # ---plot at set time----------
        ...
        _ = plot_time(
            ...
            title=title,
            baseunit=baseunit,
            time_width_percent=time_width_percent,
            lines_are="data",
            scattercut=scattercut,
            bordercut=bordercut,
            intensity_range=intensity_range,
    ...
    if 4 in plotting:  # ---matrix with fit and error, as figures----------
        fig5 = plot2d_fit(
            ...
            intensity_range=intensity_range,
            baseunit=baseunit,
            error_matrix_amplification=error_matrix_amplification,
            wave_nm_bin=None,
            equal_energy_bin=equal_energy_bin,
            use_images=True,
            log_scale=log_scale,
    ...
    if 5 in plotting:
        ...
        for i in range(4):
            ax6[i] = re["c"].plot(
                ax=ax6[i], color=colm(range(n_colors), cmap=cmap), legend=False
            )
        ...
        ax6[5].text(
            0,
            0,
            'This factor represents the temporal evolution\n of the components in the fit.\nThis time dependent factor multiplied with the \nspectral intensity from the SAS/DAS is re["AC"]',
            fontsize=float(plt.rcParams["legend.fontsize"]) - 1,
        )
        fig6.tight_layout()
    if 6 in plotting:  # ---residuals----------
        ...
        for t in times:
            if isinstance(t, float):
                ax7.plot(
                    [t, t], ax7.get_ylim(), lw=0.5, zorder=10, alpha=0.5, color="black"
                )
        fig7.tight_layout()
    plt.ion()
    plt.show()
    ...
    if save_figures_to_folder:
        ...
        for a in range(7):
            try:
                eval("fig%i" % (a + 1)).savefig(
                    check_folder(
                        path=figure_path,
                        filename="%s_%s.%s" % (fi, name_extension[a], savetype),
                    ),
                    bbox_inches="tight",
                )
            except Exception as e:
...
def plot_raw(
    ...
    wave_nm_bin=None,
    width=10,
    rel_wave=np.arange(400, 900, 100),
    rel_time=[1, 5, 10],
    time_width_percent=10,
    ignore_time_region=None,
    time_bin=None,
    ...
    """
    if ds is None:
        raise ValueError("We need something to plot!!!")
    if baseunit != "ps":
        if baseunit == "ns":
...
def plot_time(
    ...
    if "smoothed" in lines_are:
        if scattercut is None:
            ...
            if equal_energy_bin is not None:
                ...
                scattercut = scattercut[::-1]
            smoothed = Frame_golay(
                ds.loc[: scattercut[0], :], window=5, order=3, transpose=False
            )
            smoothed.plot(ax=ax1, style="-", color=colors, legend=False, lw=linewidth)
            smoothed = Frame_golay(
                ds.loc[scattercut[1] :, :], window=5, order=3, transpose=False
            )
            smoothed.plot(
                ax=ax1,
                style="-",
            ...
            try:
                ...
                for i in range(len(scattercut)):
                    if i == 0:
                        ...
                        smoothed.plot(
                            ax=ax1, style="-", color=colors, legend=False, lw=linewidth
                        )
                    elif i < (len(scattercut) / 2):
                        smoothed = Frame_golay(
                            ds.loc[scattercut[2 * i - 1] : scattercut[2 * i], :],
                            window=5,
                            order=3,
                            transpose=False,
                        )
                ...
                print(
                    "printing the smoothed scatter interpolation created an error, using default"
                )
        ...
        if scattercut is None:
            ds.plot(
                ax=ax1, style="-", color=colors, legend=False, lw=linewidth, alpha=0.7
            )
        elif isinstance(scattercut[0], numbers.Number):
            if equal_energy_bin is not None:
                scattercut = [
            ...
            ds.loc[: scattercut[0], :].plot(
                ax=ax1, legend=False, style="-", color=colors, alpha=0.7, lw=linewidth
            )
            ds.loc[scattercut[1] :, :].plot(
                ax=ax1,
                legend=False,
                style="-",
                color=colors,
            ...
            try:
                ...
                for i in range(len(scattercut)):
                    if i == 0:
                        ...
                        ds.loc[scattercut[2 * i - 1] : scattercut[2 * i], :].plot(
                            ax=ax1,
                            legend=False,
                            style="-",
                            color=colors,
    ...
    if not subplot:
        ...
        ax1.plot(
            ax1.get_xlim(), [0, 0], color="black", lw=0.5, zorder=0, label="_nolegend_"
        )
        if intensity_range is None:
...
def plot1d(
    ...
    if isinstance(cmap, list):
        ...
        colors = colm(
            np.arange(color_offset, len(wavelength) + color_offset), cmap=cmap
        )
    ds = sub_ds(
        ds=ds,
        wavelength=wavelength,
        wavelength_bin=width,
        scattercut=scattercut,
        ignore_time_region=ignore_time_region,
        drop_ignore=True,
        from_fit=from_fit,
    ...
    if "smoothed" in lines_are:
        if ignore_time_region is None:
            ...
            smoothed = Frame_golay(
                ds.loc[: ignore_time_region[0], :], window=5, order=3
            )
            smoothed.plot(ax=ax1, style="-", color=colors, legend=False, lw=linewidth)
            smoothed = Frame_golay(
                ds.loc[ignore_time_region[1] :, :], window=5, order=3
            )
            smoothed.plot(
                ax=ax1,
                style="-",
            ...
            try:
                ...
                for i in range(len(ignore_time_region) / 2 + 1):
                    if i == 0:
                        ...
                        smoothed = Frame_golay(
                            ds.loc[
                                ignore_time_region[2 * i - 1] : ignore_time_region[
                                    2 * i
                                ],
                                :,
                            ],
        ...
        if subplot:
            ...
            ds.plot(ax=ax1, style="*", color=colors, legend=False)
    elif "fitted" in lines_are:
        if ignore_time_region is None:
            ds.plot(ax=ax1, style="-", color=colors, legend=False, lw=linewidth)
        elif isinstance(ignore_time_region[0], numbers.Number):
            ds.loc[: ignore_time_region[0], :].plot(
                ax=ax1, style="-", color=colors, legend=False, lw=linewidth
            )
            ds.loc[ignore_time_region[1] :, :].plot(
                ax=ax1,
                style="-",
                color=colors,
                legend=False,
            ...
            try:
                ...
                for i in range(len(ignore_time_region) / 2 + 1):
                    if i == 0:
                        ...
                        ds.loc[
                            ignore_time_region[2 * i - 1] : ignore_time_region[2 * i], :
                        ].plot(
                            ax=ax1,
                            style="-",
                            color=colors,
    ...
    if not subplot:
        ...
        if "smoothed" in lines_are:
            leg = ax1.legend(
                labels,
                title="%g %s width, lines=smoothed" % (width, units),
                labelspacing=0,
                ncol=2,
                columnspacing=1,
                handlelength=1,
            ...
            leg = ax1.legend(
                labels,
                title="%g %s width, lines=fit" % (width, units),
                labelspacing=0,
                ncol=2,
                columnspacing=1,
                handlelength=1,
    ...
    # limits and ticks
    if timelimits is None:
        timelimits = [min(x), max(x)]
    if "symlog" in plot_type:
        lintresh = lintresh
        ax1.set_xscale(
            "symlog", linthresh=lintresh, subs=range(2, 9), linscale=lintresh / 4.0
        )
        try:
            if lintresh > 0.5:
                ticks = np.concatenate(
...
def SVD(
    ...
    if plotting:
        ...
        if title is not None:
            ...
            ax2.set_title(
                "Spectral component", fontsize=plt.rcParams["figure.titlesize"] - 4
            )
            ax3.set_title(
                "Temporal development\nof spectral component",
                fontsize=plt.rcParams["figure.titlesize"] - 4,
            )
        s /= s.max()
        ...
        ax1.set_ylabel(
            "Singular values norm.", fontsize=plt.rcParams["axes.labelsize"] - 2
        )
        ...
        ax2.legend(
            frameon=False, labelspacing=0, borderpad=0, numpoints=1, handlelength=1
        )
        ax3.legend(
            frameon=False,
            fontsize=11,
...
def Fix_Chirp(
    ...
    ds = ds.fillna(0)
    if (
        fitcoeff is not None
    ):  # we loaded a previous project this is a dublication, but I'm currently to lazy to make this tighter
        ...
        if save_file is None:
            ...
            for repeat in range(30):
                ...
                ax.add_patch(
                    matplotlib.patches.Rectangle(
                        (ax.get_xlim()[0], shown_window[1] - 0.1),
                        w,
                        0.1,
                        facecolor="white",
                    )
                ...
                for i, ent in enumerate(intensities):
                    ax.text(
                        ax.get_xlim()[0] + i * w / 10.0,
                        shown_window[1] - 0.1,
                        "%.1g" % (2 ** np.arange(-6.0, 4, 1.0))[i],
                        fontsize=20,
                    )
                ax.add_patch(
                    matplotlib.patches.Rectangle(
                        (ax.get_xlim()[0], shown_window[0]),
                        w / 4.0,
                        0.1,
                        facecolor="white",
                    )
                ...
                ax.add_patch(
                    matplotlib.patches.Rectangle(
                        (ax.get_xlim()[0] + w * 3.0 / 4.0, shown_window[0]),
                        w / 4.0,
                        0.1,
                        facecolor="white",
                    )
                ...
                factor = int((choice[0][0] - ax.get_xlim()[0]) / (w / 10.0))

                if choice[0][1] > (shown_window[0] + window_difference):
                    intensity_range = intensities[factor]
                    print((2 ** np.arange(-6.0, 4, 1.0))[factor])
                    intensity_range = [-intensity_range, intensity_range]
                    plt.close(fig)
                    continue
                elif choice[0][1] < (
                    shown_window[0] + window_difference
                ):  # we choice to finish the choices
            ...
            for repeat in range(10):
                ...
                ax = plot2d(
                    ...
                    timelimits=shown_window,
                    intensity_range=intensity_range,
                    title="like it? %i more attempts" % (9 - repeat),
                    use_colorbar=False,
                    plot_type="linear",
                    log_scale=False,
                )
                ...
                ax.add_patch(
                    matplotlib.patches.Rectangle(
                        (ax.get_xlim()[0] + w * 3 / 4, ax.get_ylim()[0]),
                        w / 4,
                        0.2,
                        facecolor="white",
                    )
        ...
        ds_new = ds.apply(
            lambda x: np.interp(
                x=time + np.polyval(fitcoeff, float(x.name)), xp=time, fp=x
            ),
            axis=0,
            raw=False,
        )
        if save_file is None:
            ...
            for repeat in range(10):
                ...
                ax = plot2d(
                    ...
                    scattercut=scattercut,
                    bordercut=bordercut,
                    lintresh=np.max(timelimits),
                    timelimits=[-cutoff_window * 0.3, cutoff_window * 0.3],
                    intensity_range=intensity_range,
                    title="corrected select new zero",
                    plot_type="lin",
                    use_colorbar=False,
                    log_scale=False,
                )
                ...
                for l in range(0, n_decays):
                    if l > 0:
                        if infinite:
                            if l < (n_decays - 1):
                                dc[l] = (
                                    decays[l - 1] * dt * c_temp[l - 1]
                                    - decays[l] * dt * c_temp[l]
                                )
                            else:
...
def err_func(
    ...
    if log_fit:
        pardf.loc[pardf.is_rate, "value"] = pardf.loc[pardf.is_rate, "value"].apply(
            lambda x: 10**x
        )
    if isinstance(mod, type("hello")):  # did we use a build in model?
        ...
        if pulse_sample is not None:
            ...
            if pump_region.max() < times_ori.min():
                connection_region = np.arange(
                    pump_region[-1], times_ori.min(), resolution / 10
                )
                times = np.unique(
                    np.sort(np.hstack((pump_region, connection_region, times_ori)))
                )
            else:
        ...
        if sub_sample is not None:
            ...
            for i in range(1, sub_sample, 1):
                listen.append(
                    times_ori[:-1] + ((times_ori[1:] - times_ori[:-1]) * i / sub_sample)
                )
        ...
        if ext_spectra is None:
            ...
            if "ext_spectra_shift" in list(pardf.index.values):
                ext_spectra.index = (
                    ext_spectra.index.values + pardf.loc["ext_spectra_shift", "value"]
                )
            ...
            for col in ext_spectra.columns.values:
                ...
                C = pandas.DataFrame(
                    (A * B).T, index=c.index, columns=ext_spectra.index.values
                )
                ds = ds - C
        ...
        if final:
            ...
            if not ext_spectra is None:
                for col in ext_spectra.columns.values:
                    ...
                    A, B = np.meshgrid(
                        c.loc[:, col].values, ext_spectra.loc[:, col].values
                    )
                    C = pandas.DataFrame(
                        (A * B).T, index=c.index, columns=ext_spectra.index.values
                    )
                    ...
                    re["AC"] = re["AC"] + C
            re["r2"] = (
                1 - re["error"] / ((re["A"] - re["A"].mean().mean()) ** 2).sum().sum()
            )
            if dump_paras:
                try:
                ...
                try:
                    ...
                    min_df = pandas.read_csv(
                        store_name, sep=",", header=None, skiprows=1
                    )
        ...
        if final:
            ...
            if dump_paras:
                ...
                try:
                    min_df = pandas.read_csv(
                        "minimal_dump_paras.par", sep=",", header=None, skiprows=1
                    )
                    if float(min_df.iloc[-1, 1]) > float(re["error"]):
...
def err_func_multi(
    ...
    if same_DAS:
        ...
        for i, ta in enumerate(multi_project):
            ...
            ds = sub_ds(
                ds=ta.ds,
                scattercut=slice_setting_object.scattercut,
                bordercut=slice_setting_object.bordercut,
                timelimits=slice_setting_object.timelimits,
                wave_nm_bin=slice_setting_object.wave_nm_bin,
                time_bin=slice_setting_object.time_bin,
                ignore_time_region=slice_setting_object.ignore_time_region,
            )
            ...
            if log_fit:
                pardf.loc[pardf.is_rate, "value"] = pardf.loc[
                    pardf.is_rate, "value"
                ].apply(lambda x: 10**x)
            ...
            if pulse_sample is not None:
                ...
                if hasattr(pulse_sample, "__iter__"):
                    ...
                    pump_region = np.linspace(
                        t0 - 4 * resolution, t0 + 4 * resolution, 20
                    )
                if pump_region.max() < times_ori.min():
                    connection_region = np.arange(
                        pump_region[-1], times_ori.min(), resolution / 10
            ...
            if sub_sample is not None:
                ...
                for i in range(1, sub_sample, 1):
                    listen.append(
                        times_ori[:-1]
                        + ((times_ori[1:] - times_ori[:-1]) * i / sub_sample)
                    )
                times = np.unique(np.hstack(listen))
                times.sort()
            ...
            if isinstance(mod, type("hello")):  # did we use a build in model?
                ...
                c = mod(
                    times=ds.index.values.astype("float"), pardf=pardf.loc[:, "value"]
                )
            c = c.loc[times_ori, :]
            ...
            if ext_spectra is None:
                ...
                if "ext_spectra_shift" in list(pardf.index.values):
                    ext_spectra.index = (
                        ext_spectra.index.values
                        + pardf.loc["ext_spectra_shift", "value"]
                    )
                    ext_spectra = rebin(ext_spectra, ds.columns.values.astype(float))
                else:
                    ext_spectra = rebin(ext_spectra, ds.columns.values.astype(float))
        ...
        if final:
            ...
            for i, ta in enumerate(multi_project):
                ...
                re_local["AC"] = (
                    re["AC"].copy().iloc[lower : lower + height_stack[i], :]
                )
                re_local["AE"] = (
                    re["AE"].copy().iloc[lower : lower + height_stack[i], :]
                )
                ...
                re_local["r2_total"] = (
                    1
                    - re["error"] / ((re["A"] - re["A"].mean().mean()) ** 2).sum().sum()
                )
    ...
    ###################   not same DAS####################
            ...
            if isinstance(mod, type("hello")):  # did we use a build in model?
                ...
                if pulse_sample is not None:
                    ...
                    if pump_region.max() < times_ori.min():
                        ...
                        times = np.unique(
                            np.sort(
                                np.hstack((pump_region, connection_region, times_ori))
                            )
                        )
                    else:
                        times = np.unique(np.sort(np.hstack((pump_region, times_ori))))
                        ...
                        if not ext_spectra is None:
                            for col in ext_spectra.columns.values:
                                ...
                                C = pandas.DataFrame(
                                    (A * B).T,
                                    index=c.index,
                                    columns=ext_spectra.index.values,
                                )
                                re["A"] = re["A"] + C
                                re["AC"] = re["AC"] + C
                        re_final = re.copy()
        ...
        if not weights is None:
            ...
            combined_error = np.sqrt(
                ((np.array(error_listen) * np.array(weights)) ** 2).mean()
            )
            if final:
                combined_r2 = np.sqrt(
                    ((np.array(r2_listen) * np.array(weights)) ** 2).mean()
                )
        else:
    ...
    if "upper_limit" in pardf.keys():
            ...
            for row in pardf.index.values:
                if timedf.loc[row, "is_rate"]:
                    ...
                    try:
                        ...
                        if key == "init_value":
                            pass  # we don't save the init values, so we get an error when converting the saved file
                        elif pardf.loc[row, key] is None:
                            continue
                        else:
                            print("conversion of this key failed: %s %s" % (row, key))
    else:
        for key in ["init_value", "value", "min", "max"]:
            if key == "min":
                ...
                key_in = key
            try:
                timedf.loc[pardf.is_rate, key_in] = pardf.loc[pardf.is_rate, key].apply(
                    lambda x: 1 / x if x != 0 else "inf"
                )
            except:
                if key == "init_value":
                    pass  # we don't save the init values, so we get an error when converting the saved file
                else:
                    print("conversion of this key failed:" + key)
    return timedf
...
class TA:  # object wrapper for the whole
    def __init__(
        ...
        if filename == "gui" and not ext_data_switch:
            ...
            try:
                ...
                with open("recent.dat", "w") as f:
                    f.write(complete_path)
        if (
            filename == "external" or ext_data_switch
        ):  # use a provided dataframe (ds) instead
        ...
        elif ("hdf5" in filename) and (
            conversion_function is None
        ):  # we load a conversion function to deal with the file:#we read in data from previous run
            self.__read_project(
                saved_project=check_folder(path=self.path, filename=self.filename)
            )
            self.__make_standard_parameter()
            ...
            if conversion_function is not None:
                ...
                try:
                    ...
                    try:
                        ret = conversion_function(
                            filename=filename, external_time=external_time
                        )
                    except:
                        try:
                            ret = conversion_function(
                                filename=filename, external_wave=external_wave
                            )
                        except:
                            try:
                ...
                if isinstance(ret, pandas.DataFrame):
                    ...
                    if len(ret) == 2:
                        ...
                        print(
                            "sorry the return format of the conversion_function was not understood"
                        )
                        print(
                            "return: the dataframe ds with the time_axis as rows and spectral axis as columns\n"
                        )
                        ...
                        print(
                            "if three are returned the third is the baseunit (for the time axis) this allows to use the automatic naming in ps or ns "
                        )
                        return False
    ...
    def __read_ascii_data(
        ...
        self.ds_ori = pandas.read_csv(
            check_folder(path=self.path, filename=self.filename), sep=sep, index_col=0
        )
        if correct_ascii_errors:
            if (self.ds_ori.applymap(type) == float).all().all():
                ...
                try:  # try forced conversion
                    self.ds_ori = self.ds_ori.applymap(
                        lambda x: re.sub("--", "-", x) if type(x) is str else x
                    )
                    self.ds_ori = self.ds_ori.applymap(
                        lambda x: re.sub(r"\.+", ".", x) if type(x) is str else x
                    )
                    self.ds_ori = self.ds_ori.astype(np.float64)
                except Exception as e:
                    print(
                        "force cleaning went wrong and the file %s can not be read. Error message is:"
                        % self.filename
                    )
                    print(e)
                    return False
        if external_time is not None:
            if use_same_name:
                time_file = check_folder(
                    path=self.path,
                    filename=self.filename.split(".")[0] + "." + external_time,
                )
            else:
                time_file = check_folder(path=self.path, filename=external_time)
        if external_wave is not None:
            if use_same_name:
                wave_file = check_folder(
                    path=self.path,
                    filename=self.filename.split(".")[0] + "." + external_wave,
                )
            else:
        ...
        if external_time is not None:
            ...
            times = pandas.read_csv(
                time_file, header=None, decimal=decimal
            ).values.ravel()
            if transpose:
                if external_wave is not None:
                    self.ds_ori = pandas.read_csv(
                        check_folder(path=self.path, filename=self.filename),
                        sep=sep,
                        decimal=decimal,
                        header=None,
                    )
                    waves = pandas.read_csv(
                        wave_file, header=None, decimal=decimal
                    ).values.ravel()
            ...
            if transpose:
                ...
                self.ds_ori.columns = waves
        self.ds_ori.columns = self.ds_ori.columns.astype(
            "float"
        )  # Make columns indexes numbers
        self.ds_ori.index = self.ds_ori.index.astype(
            "float"
        )  # Make row indexes numbers
        if index_is_energy:
            self.ds_ori.index = (
                scipy.constants.h
                * scipy.constants.c
                / (self.ds_ori.index * 1e-9 * scipy.constants.electron_volt)
            )
    ...
    def __make_standard_parameter(self):
        ...
        self.ignore_time_region = (
            None if not hasattr(self, "ignore_time_region") else self.ignore_time_region
        )
        self.error_matrix_amplification = (
            10
            if not hasattr(self, "error_matrix_amplificatio")
            else self.error_matrix_amplification
        )
        try:
            ...
            self.rel_wave = (
                ...
                if not hasattr(self, "rel_wave")
                else self.rel_wave
            )
        self.rel_time = (
            [0.2, 0.3, 0.5, 1, 3, 10, 30, 100, 300, 1000, 3000, 9000]
            if not hasattr(self, "rel_time")
            else self.rel_time
        )
        self.time_width_percent = (
            0 if not hasattr(self, "time_width_percent") else self.time_width_percent
        )
        ...
        self.wave_nm_bin = (
            None if not hasattr(self, "wave_nm_bin") else self.wave_nm_bin
        )
        self.wavelength_bin = (
            10 if not hasattr(self, "wavelength_bin") else self.wavelength_bin
        )
        self.save_figures_to_folder = (
            False
            if not hasattr(self, "save_figures_to_folder")
            else self.save_figures_to_folder
        )
        self.intensity_range = (
            None if not hasattr(self, "intensity_range") else self.intensity_range
        )
        self.ds_ori.index.name = (
            "Time in %s" % self.baseunit
            if not hasattr(self, "ds_ori.index.name")
            else self.ds_ori.index.name
        )
        self.equal_energy_bin = (
            None if not hasattr(self, "equal_energy_bin") else self.equal_energy_bin
        )
        self.units = "nm" if not hasattr(self, "units") else self.units
        if self.units == "nm":
            self.ds_ori.columns.name = (
                "Wavelength in %s" % self.units
                if not hasattr(self, "ds_ori.columns.name")
                else self.ds_ori.columns.name
            )
        elif self.units == "eV":
            self.ds_ori.columns.name = (
                "Energy in %s" % self.units
                if not hasattr(self, "ds_ori.columns.name")
                else self.ds_ori.columns.name
            )
        elif self.units == "keV":
        ...
        self.data_type = (
            "diff. Absorption in $\mathregular{\Delta OD}$"
        ...
        try:  # self.chirp_file
            ...
            if os.path.isfile(
                check_folder(
                    path=self.path, filename=self.filename.split(".")[0] + "_chirp.dat"
                )
            ):
        ...
        try:  # self.figure_path
            ...
            if self.save_figures_to_folder:
                self.figure_path = check_folder(
                    path="result_figures", current_path=self.path
                )
            else:
    ...
    def Filter_data(
        ...
        if uppervalue is None:
            uppervalue = np.abs(value)
        if lowervalue is None:
            lowervalue = -np.abs(value)
        ...
        for dataset in filtering:
            if any(
                [
                    self.ignore_time_region is not None,
                    self.scattercut is not None,
                    self.bordercut is not None,
                    self.timelimits is not None,
                ]
            ):
                dataset = sub_ds(
                    dataset,
                    ignore_time_region=self.ignore_time_region,
                    scattercut=self.scattercut,
                    bordercut=self.bordercut,
                    timelimits=self.timelimits,
                )
            if (
                cut_bad_times
            ):  # timepoint filter, delete the timepoints where value is stupid
                matrix_size = len(dataset.index.values)
                if upper_matrix is None:
                    damaged_times = dataset[
                        np.any(dataset.values > uppervalue, axis=1)
                    ].index
                else:
                    damaged_times = dataset[
                        np.any(dataset.values > upper_matrix, axis=1)
                    ].index
                dataset.drop(damaged_times, inplace=True)
                if lower_matrix is None:
                    damaged_times = dataset[
                        np.any(dataset.values < lowervalue, axis=1)
                    ].index
                else:
                    damaged_times = dataset[
                        np.any(dataset.values < lower_matrix, axis=1)
                    ].index
                dataset.drop(damaged_times, inplace=True)
                ...
                if replace_bad_values == np.nan:
                    ...
                    if (
                        dataset[dataset == replace_bad_values].notna().sum().sum()
                        > 0.2
                        * dataset[dataset != replace_bad_values].notna().sum().sum()
                    ):
                        print(
    ...
    def Background(
        self, lowlimit=None, uplimit=-1, use_median=False, ds=None, correction=None
    ):
        ...
        if ds is None:
            ...
            if correction is None:
                raise ValueError("We must have correction given, to slow otherhwise")
    ...
    def Man_Chirp(
        self, shown_window=[-1, 1], path=None, max_points=40, cmap=cm.prism, ds=None
    ):
        ...
        if original:
            ...
            temp_ds = Fix_Chirp(
                ...
                wave_nm_bin=10,
                shown_window=shown_window,
                filename=self.filename + "_second_chirp",
                scattercut=self.scattercut,
                bordercut=self.bordercut,
                path=check_folder(path=path, current_path=self.path),
                max_points=max_points,
        ...
        if isinstance(temp_ds, pandas.DataFrame):
            ...
            if original:  # we have run from scratch
                ...
                print(
                    "you provided a separate ds file. returned are the new fitcoeff and the combined fitcoeff, ta also has a new variable called ta.combined_fitcoeff"
                )
                save_file = check_folder(
                    path=path,
    ...
    def Check_Chirp(
        ...
        ax = plot2d(
            ...
            timelimits=shown_window,
            intensity_range=self.intensity_range,
            title="This plot shows the set chirp",
            use_colorbar=False,
            plot_type="linear",
            log_scale=False,
        )
    ...
    def Cor_Chirp(
        ...
        if fitcoeff is not None:  # we use a stored project
            try:
                if (
                    len(fitcoeff) == 5 or len(fitcoeff) == 6
                ):  # we provide a valid list/vector
                    ...
                    self.fitcoeff = (
                        fitcoeff  # we came to here so fitcoeff must be right
                    )
                else:
                    raise
            except:
                ...
                print(
                    "something went wrong with the provided fitcoeff. This should be either a list/array with 5-6 parameter or the object should contain the parameter"
                )
            ...
            try:
                ...
                print(
                    check_folder(
                        path=self.path,
                        filename=self.filename.split(".")[0] + "_chirp.dat",
                    )
                )
                if os.path.isfile(
                    check_folder(
        ...
        class MouseMove:
            ...
            def move(self, event):
                ...
                self.ax_time.set_yticklabels(
                    ["%.1e" % f for f in self.ax_time.get_ylim()]
                )
                ...
                if not fitted:
                    ...
                    self.ax_kinetic.plot(
                        ds_temp1.values,
                        ds_temp1.index.values,
                        "-",
                        label="%.0f smoothed" % x,
                        color="red",
                    )
                ...
                self.ax_kinetic.set_xlim(
                    min([0, min(ds_temp.values)]), max([max(ds_temp.values), 0])
                )

                self.ax_kinetic.plot(
                    ds_temp.values,
                    ds_temp.index.values,
                    "*",
                    label="%.0f" % x,
                    color="black",
                )
                ...
                self.ax_kinetic.set_xticklabels(
                    ["%.1e" % f for f in self.ax_kinetic.get_xlim()]
                )
    ...
    def Save_Plots(
        ...
        if savetype is None:
            savetype = ["png"]
        elif savetype in ["png", "pdf", "svg"]:
            savetype = [savetype]
        elif hasattr(savetype, "__iter__"):
            savetype = list(savetype)
        else:
            print(
                "Please specify a single filetype from 'png','pdf','svg' or a list of those. Nothing was saved"
            )
            return False
        if cmap is None:
            cmap = standard_map
        ...
        try:
            for t in savetype:
                ...
                plt.close("all")
                print(
                    "saved RAW plots type %s to %s"
                    % (t, check_folder(path=path, current_path=self.path))
                )
        except:
        ...
        try:
            for t in savetype:
                ...
                plt.close("all")
                print(
                    "Saved Fit plots of type %s to %s"
                    % (t, check_folder(path=path, current_path=self.path))
                )
        except:
    ...
    def __Fit_Chirp_inner(
        self,
        opt_coeff,
        initial_fit_coeff=None,
        params=None,
        scattercut=None,
        bordercut=None,
        timelimits=None,
        ...
        ds_new = ds_back_corr.apply(
            lambda x: np.interp(x=time + np.polyval(fitcoeff, x.name), xp=time, fp=x),
            axis=0,
            raw=False,
        )
    ...
    def __Fit_Chirp_outer(
        self, pardf, results, fit_ds, fit_chirp_iterations, mod, deep_iteration=False
    ):
        ...
        for loop in range(fit_chirp_iterations):
            ...
            chirpmini = lmfit.Minimizer(
                self.__Fit_Chirp_inner,
                chirp_par,
                fcn_kws={
                    "ds_back_corr": ds_back_corr.copy(),
                    "initial_fit_coeff": initial_fit_coeff,
                    "params": par_into_chirpfit,
                    "mod": self.mod,
                    "log_fit": self.log_fit,
                    "scattercut": self.scattercut,
                    "bordercut": self.bordercut,
            ...
            try:
                ...
                chirp_results = chirpmini.minimize(
                    "nelder",
                    options={
                        "maxfev": 1e4,
                        "fatol": initial_error[-1] * 1e-6,
                ...
                time = ds_back_corr.index.values.astype("float")  # extract the time
                new_ds = ds_back_corr.copy().apply(
                    lambda x: np.interp(
                        x=time + np.polyval(temp, float(x.name)), xp=time, fp=x
                    ),
                    axis=0,
                    raw=False,
                )
                ...
                if pardf.vary.any():
                    ...
                    initial_error.append(
                        err_func(
                            paras=par_into_chirpfit,
                            ds=fit_ds_loop,
                            mod=mod,
                            final=False,
                            log_fit=self.log_fit,
                ...
                if initial_error[-1] < initial_error[-2]:
                    if (
                        initial_error[-2] / initial_error[-1] > 100
                    ):  # something fishy going on. lets try again
                        print(
                            "Chirp_loop {:02d} strange decrease step size".format(
                                loop + 1
                            )
                        )
                        ...
                        if len(initial_error) > 4:
                            if (
                                initial_error[-4] == initial_error[-1]
                            ):  # we have run this trick now three times,  time to break
                                raise StopIteration
                        deep_iteration = False
                    else:
                        print(
                            "Chirp_loop {:02d} resulted in   :{:.8e}".format(
                                loop + 1, initial_error[-1]
                            )
                        )
                        if (
                            deep_iteration
                        ):  # This results in a very deep iteration of the starting parameter
                            if pardf.vary.any():
                ...
                initial_error.append(
                    initial_error[0]
                )  # to avoid that numbers are written
                break
        ...
        if initial_error[-1] < initial_error[0]:  # lets check if we improved anything
            print(
                "chirp fit improved error by %.2g percent"
                % (100 * (1 - initial_error[-1] / initial_error[0]))
            )
    ...
    def Fit_Global(
        ...
        use_ampgo=False,
        other_optimizers=None,
        fit_chirp=False,
        fit_chirp_iterations=10,
        multi_project=None,
        unique_parameter=None,
        weights=None,
        same_DAS=False,
        ...
        pardf.loc[
            np.logical_and(pardf.loc[:, "min"].values < 0, pardf.is_rate), "min"
        ] = 0
        pardf.loc[
            np.logical_and(pardf.loc[:, "max"].values < 0, pardf.is_rate), "max"
        ] = 0
        ...
        if self.log_fit:
            for key in ["value", "min", "max"]:
                pardf.loc[pardf.is_rate, key] = pardf.loc[pardf.is_rate, key].apply(
                    lambda x: np.log10(x)
                )
        ...
        if multi_project is None:
            ...
            if (filename is None) and dump_shapes:
                filename = self.filename
                    ...
                    if not use_ampgo:
                        if len(pardf[pardf.vary].index) > 3:
                            ...
                            results = mini.minimize(
                                "nelder", tol=tol, options={"adaptive": True}
                            )
                        else:
        ...
        ##########################################################################
            ...
            if (
                pardf.vary.any()
            ):  # ok we have something to optimize lets return the spectra
                ...
                mini = lmfit.Minimizer(
                    ...
                    fcn_kws={
                        ...
                        "sub_sample": sub_sample,
                        "pulse_sample": pulse_sample,
                        "same_shape_params": same_shape_params,
                    },
                )
                if other_optimizers is None:
                    if len(pardf[pardf.vary].index) > 3:
                        print("we use adaptive mode for nelder")
                        results = mini.minimize(
                            "nelder", options={"adaptive": True, "fatol": tol}
                        )
                    else:
                        results = mini.minimize("nelder", options={"fatol": tol})
                else:
        ...
        if self.ignore_time_region is not None:
            if fit_chirp:
                print(
                    "sorry but currently you can not both ignore a time region and fit the chirp (assuming that you ignore the time-zero region)"
                )
            fit_chirp = False
        if fit_chirp:
            print("Done initial fitting now chirpfit")
            results, fit_ds = self.__Fit_Chirp_outer(
        ...
        if (
        ...
        ):  # ok we calculate errors to the level of the confidence_level
            if self.log_fit:
                for key in ["value", "min", "max"]:
                    pardf.loc[pardf.is_rate, key] = pardf.loc[pardf.is_rate, key].apply(
                        lambda x: np.log10(x)
            ...
            if pardf.vary.any():  # we actually have optimised something
                if (0.6 < confidence_level < 1) or (1 < confidence_level < 0.6):
                    if multi_project is None:
                        target = s2_vs_smin2(
                            Spectral_points=len(re["A"].columns),
                            Time_points=len(re["A"].index),
                            number_of_species=len(re["DAC"].columns),
                            fitted_kinetic_pars=len(pardf[pardf.vary].index),
                            target_quality=confidence_level,
                        )
                    else:
                        multi_project.insert(0, self)
                        # we assume that we have the same number of spectal points but are stacking the times
                        total_time_points = np.array(
                            [len(t.re["A"].index) for t in multi_project]
                        ).sum()
                        target = s2_vs_smin2(
                            Spectral_points=len(re["A"].columns),
                    ...
                    for fixed_par in list_of_variable_parameter:
                        ...
                        for i in ["lower", "upper"]:
                            print(
                                "Trying to find %s, %s confidence limit"
                                % (fixed_par, i)
                            )
                            ...
                            if "lower" in i:  # go below min
                                if par_to_pardf(pardf_local).loc[fixed_par, "is_rate"]:
                                    par_local.add(
                                        fixed_par,
                                        value=pardf_local[fixed_par].value * 0.95,
                                        min=0,
                                        max=pardf_local[fixed_par].value,
                                        vary=vary_error_parameter,
                                    )
                                else:
                                    par_local.add(
                                        fixed_par,
                                ...
                                par_local.add(
                                    fixed_par,
                                    value=pardf_local[fixed_par].value * 1.05,
                                    min=pardf_local[fixed_par].value,
                                    vary=vary_error_parameter,
                                )
                            ...
                            def sub_problem(
                                ...
                                pardf_local[varied_par].value = par_local[
                                    varied_par
                                ].value
                                if par_to_pardf(pardf_local).vary.any():
                                    if multi_project is None:
                                        mini_sub = lmfit.Minimizer(
                                            err_func,
                                            pardf_local,
                                            fcn_kws={
                                                "ds": fit_ds,
                                    ...
                                    local_error = (
                                        results_sub.residual[0] - target_s2
                                    ) ** 2
                                    return local_error
                                else:
                                    if multi_project is None:
                            ...
                            try:
                                ...
                                mini_local = lmfit.Minimizer(
                                    ...
                                    par_local,
                                    fcn_kws={
                                        "varied_par": fixed_par,
                                        "pardf_local": pardf_local,
                                        "fit_ds": fit_ds,
                                        "multi_project": multi_project,
                                        "unique_parameter": unique_parameter,
                                        "same_DAS": same_DAS,
                                ...
                                one_percent_precission = (
                                    (target - 1) * 0.01 * re["error"]
                                )
                                # results_local = mini_local.minimize('least_squares',ftol=one_percent_precission)
                                if other_optimizers is None:
                                    results_local = mini_local.minimize(
                                        method="nelder",
                                        options={
                                            "maxiter": 100,
                                            "fatol": one_percent_precission,
                                        },
                                    )
                                else:
                                    results_local = mini_local.minimize(
                                        method=other_optimizers,
                                        options={
                                            "maxiter": 100,
                                            "fatol": one_percent_precission,
                                        },
                                ...
                                if results_local.success:
                                    conf_limits[fixed_par][i] = results_local.params[
                                        fixed_par
                                    ].value
                                else:
                                    print(
                                        "tried to optimise %i times achieved residual %g with targeted %g"
                                        % (
                                            results_local.nfev,
                                            (
                                                np.sqrt(results_local.residual[0])
                                                + target_s2
                                            ),
                                            target_s2,
                                        )
                    ...
                    return False
                print(
                    "it took %i optimisations to get the confidence" % iterative_calls
                )
        ...
        if pardf.vary.any():
            re["fit_output"] = (
                results  # let's store the fit results in the re_object for now.
            )
                        ...
                        for row in pardf[pardf.is_rate].index.values:
                            try:
                                ...
                                if pardf.loc[row, key] is None:
                                    ...
                                    print(
                                        "%s,%s has could not be converted and has value"
                                        % (row, key)
                                    )
        ...
        if 1:
            if self.equal_energy_bin is not None:
                if same_DAS:
                    for i, re_local in enumerate(re_listen):
                        for name in ["A", "AC", "AE"]:
                            re_local[name].columns = (
                                scipy.constants.h
                                * scipy.constants.c
                                / (
                                    re_local[name].columns.values
                                    * 1e-9
                                    * scipy.constants.electron_volt
                                )
                            )
                            re_local[name].columns.name = "wavelength in nm"
                            re_local[name].sort_index(
                                inplace=True, axis=1, ascending=True
                            )
                        re_local["DAC"].index = (
                            scipy.constants.h
                            * scipy.constants.c
                            / (
                                re_local["DAC"].index.values
                                * 1e-9
                                * scipy.constants.electron_volt
                            )
                        )
        ...
        if self.ignore_time_region is not None:
            try:
                Result_string += (
                    "the time between %.3f %s and %.3f %s was excluded from the optimization\n\n"
                    % (
                        self.ignore_time_region[0],
                        self.baseunit,
                        self.ignore_time_region[1],
                        self.baseunit,
                    )
                )
        ...
        if save_RAW:
            self.ds.to_csv(
                check_folder(
                    path=path,
                    current_path=self.path,
                    filename=filename + "_chirp_corrected_raw_matrix.dat",
                ),
                sep=sep,
            )
            if save_binned:
                ...
                sub.to_csv(
                    check_folder(
                        path=path,
                        current_path=self.path,
                        filename=filename + "_chirp_corrected_rebinned_matrix.dat",
                    ),
                    sep=sep,
                )
            if save_slices:
                sub = sub_ds(
                    ds=self.ds.copy(),
                    wavelength_bin=self.wavelength_bin,
                    wavelength=self.rel_wave,
                )
                ...
                sub.to_csv(
                    check_folder(
                        path=path,
                        current_path=self.path,
                        filename=filename + "_chirp_corrected_RAW_kinetics.dat",
                    ),
                    sep=sep,
                )
                sub = sub_ds(
                ...
                sub.to_csv(
                    check_folder(
                        path=path,
                        current_path=self.path,
                        filename=filename + "_chirp_corrected_RAW_Spectra.dat",
                    ),
                    sep=sep,
                )
        if save_Fit:
        ...
        if save_Fit:
            self.re["A"].to_csv(
                check_folder(
                    path=path,
                    current_path=self.path,
                    filename=filename + "_matrix used as fit input.dat",
                ),
                sep=sep,
            )
            self.re["AC"].to_csv(
                check_folder(
                    path=path,
                    current_path=self.path,
                    filename=filename + "_matrix calculated during fit.dat",
                ),
                sep=sep,
            )
            self.re["AE"].to_csv(
                check_folder(
                    path=path,
                    current_path=self.path,
                    filename=filename + "_error_matrix calculated during fit.dat",
                ),
                sep=sep,
            )
            if save_slices:
                ...
                sub.to_csv(
                    check_folder(
                        path=path,
                        current_path=self.path,
                        filename=filename + "_fitted_kinetics.dat",
                    ),
                    sep=sep,
                )
                sub = sub_ds(
                ...
                sub.to_csv(
                    check_folder(
                        path=path,
                        current_path=self.path,
                        filename=filename + "_measured_kinetics.dat",
                    ),
                    sep=sep,
                )
                sub = sub_ds(
                ...
                sub.to_csv(
                    check_folder(
                        path=path,
                        current_path=self.path,
                        filename=filename + "_fitted_spectra.dat",
                    ),
                    sep=sep,
                )
                sub = sub_ds(
                ...
                sub.to_csv(
                    check_folder(
                        path=path,
                        current_path=self.path,
                        filename=filename + "_measured_spectra.dat",
                    ),
                    sep=sep,
                )
                self.re["DAC"].to_csv(
                    check_folder(
                        path=path,
                        current_path=self.path,
                        filename=filename + "_DAS-SAS.dat",
                    ),
                    sep=sep,
                )
            ...
            if save_fit_results:
                ...
                if "confidence" in self.re:
                    ...
                    Result_string += self.re["fit_results_rates"].to_string(
                        columns=[
                            "value",
                            "lower_limit",
                            "upper_limit",
                    ...
                    Result_string += self.re["fit_results_times"].to_string(
                        columns=[
                            "value",
                            "lower_limit",
                            "upper_limit",
                    ...
                    Result_string += "\nIn Rates\n"
                    Result_string += self.re["fit_results_rates"].to_string(
                        columns=["value", "init_value", "vary", "min", "max", "expr"]
                    )
                    Result_string += (
                        "\n\nThe rates converted to times with unit %s\n"
                        % self.baseunit
                ...
                with open(
                    check_folder(
                        path=path,
                        current_path=self.path,
                        filename=filename + "_fit_results_parameter.par",
                    ),
                    "w",
                ) as text_file:
                    text_file.write(Result_string)
    ...
    def Save_Powerpoint(
        ...
        """
        if isinstance(savetype, type("hello")):
            savetype = [savetype]
        if not hasattr(savetype, "__iter__"):
            savetype = [savetype]
        ...
        raw_names = [
            check_folder(
                current_path=self.path,
                path=path,
                filename=self.filename.split(".")[0] + "_RAW_" + str(a) + ".png",
            )
            for a in raw_names
        ]
        ...
        fit_names = [
            check_folder(
                current_path=self.path,
                path=path,
                filename=self.filename.split(".")[0] + "_" + str(a) + ".png",
            )
            for a in fit_names
        ]
        plt.close("all")
        ...
        if save_RAW:
            self.save_figures_to_folder = True
            self.Plot_RAW(
                savetype="png", scale_type=scale_type, title=title, cmap=cmap, path=path
            )
            plt.close("all")
        if save_Fit:
            try:
                ...
                save_Fit = False
                print(
                    "run into problems with adding the fit results. Have you fitted something?"
                )
            try:
            ...
            try:
                ...
                if save_RAW:
                    ...
                    pic = slide.shapes.add_picture(
                        str(raw_names[0].resolve()),
                        left=left + Inches(4.5),
                        top=top,
                        width=Inches(4.5),
                    )
                    pic = slide.shapes.add_picture(
                        str(raw_names[1].resolve()),
                        left=left,
                        top=top,
                        width=Inches(4.5),
                    )
                    pic = slide.shapes.add_picture(
                        str(raw_names[2].resolve()),
                        left=left,
                        top=top + Inches(3),
                        width=Inches(4.5),
                    )
                ...
                if save_Fit:
                    try:
                        ...
                        pic = slide2.shapes.add_picture(
                            str(fit_names[0].resolve()),
                            left=left + Inches(7.0),
                            top=top,
                            height=Inches(3.4),
                        )  # Matrix
                        pic = slide2.shapes.add_picture(
                            str(fit_names[1].resolve()),
                            left=left,
                            top=top,
                            height=Inches(2),
                        )
                        pic = slide2.shapes.add_picture(
                            str(fit_names[2].resolve()),
                            left=left,
                            top=top + Inches(2),
                            height=Inches(2),
                        )
                        pic = slide2.shapes.add_picture(
                            str(fit_names[3].resolve()),
                            left=left,
                            top=top + Inches(3.9),
                            height=Inches(1.4),
                        )
                        pic = slide2.shapes.add_picture(
                            str(fit_names[4].resolve()),
                            left=left,
                            top=top + Inches(5.4),
                            height=Inches(2),
                        )
                        text1 = slide2.shapes.add_textbox(
                            left=left + Inches(5.2),
                            top=top + Inches(2.5),
                            width=Inches(4.5),
                            height=Inches(4.5),
                        )
                ...
                prs.save(
                    check_folder(
                        path=path,
                        current_path=self.path,
                        filename=self.filename.split(".")[0] + ".pptx",
                    )
                )
                print(
                    "The images and a powerpoint was saved to %s"
                    % check_folder(path=path, current_path=self.path)
                )
            except Exception as e:
                print(
                    "Error in powerpoint generation. Most likely a module is missing."
                )
                print(
                    'We need python-pptx to create a powerpoint file.  Either use "pip install python-pptx" or "conda install -c conda-forge python-pptx" '
                )
                print(
                    "We will save the results as pdf format for now. Check th error if somehting else went wrong"
                )
                print(e)
                savetype.append("pdf")
    ...
    def Save_project(self, filename=None, path=None):
        ...
        if filename is None:
            filename = self.filename
        hdf5_name = check_folder(
            path=path, current_path=self.path, filename=filename.split(".")[0] + ".hdf5"
        )
                ...
                if key == "mod":
                    if self.__dict__[key] in [
                        ...
                        try:
                            ...
                            f.create_dataset(
                                name=key, data="external_function_without_docstring"
                            )
                elif key in [
                    "rel_wave",
                    "rel_time",
                ]:  # need extra, as it is bypassed by the re-switch
                    f.create_dataset(name=key, data=self.__dict__[key])
                elif key[:2] == "re":
                    re_switch = True
                    for key2 in self.__dict__["re"]:
                        ...
                        if key2 == "error":
                            ...
                            for key3 in self.__dict__["re"]["confidence"].keys():
                                try:
                                    f.create_dataset(
                                        name="re_confidence_%s_upper" % key3,
                                        data=self.__dict__["re"][key2][key3]["upper"],
                                    )
                                    f.create_dataset(
                                        name="re_confidence_%s_lower" % key3,
                                        data=self.__dict__["re"][key2][key3]["lower"],
                                    )
                                except:
                                    try:
                                        f.create_dataset(
                                            name="re_confidence_%s" % key3,
                                            data=self.__dict__["re"][key2][key3],
                                        )
                                    except:
                                        print(
                                            "saving of" + key3 + "in confidence failed"
                                        )
                            ...
                            try:
                                f.create_dataset(
                                    name="re_" + key2, data=self.__dict__["re"][key2]
                                )
                            except:
                    ...
                    f.create_dataset(
                        name="back", data=self.__dict__["background_par"][3]
                    )
                    ...
                    df = par_to_pardf(
                        self.__dict__[key]
                    )  # pandas has a bug and problems handling mixed type columns when saving. So we clean up.
            ...
            if not "fitcoeff" in f:
                try:
                    ...
                    try:
                        ...
                        pass
        self.ds_ori.to_hdf(
            str(hdf5_name.resolve()), key="ds_ori", append=True, mode="r+", format="t"
        )  # save_raw_data
        if re_switch:
            # print('re-switched')
    ...
    def __read_project(self, saved_project=None, current_path=None):
        ...
        if current_path is None:
            current_path = os.path.dirname(os.path.abspath(saved_project))
        try:
            import h5py
        except:

            print(
                "could not import hdf5, current version requires that this is installed. IF running Anaconda open Conda promt and type: conda install h5py"
            )
        data_frame_list = []

        # we hav to handle the old and new type of saving
        with h5py.File(saved_project, "r") as f:
            if "re_final_setup_par" in f.keys():
                old_switch = True
        ...
        if old_switch:
            print(
                "we read an old style data_file and directly update it into the new file_type after loading"
            )
        with h5py.File(saved_project, "r") as f:
            for key in f.keys():
                try:
        ...
        for key in data_frame_list:
            try:
                if "re_" in key[:3]:
                    # print('re in list')
                    self.__dict__["re"][key[3:]] = pandas.read_hdf(
                        saved_project, key=key, mode="r", data_columns=True
                    )
                elif key in ["ds_ori", "par_fit", "par"]:
                    self.__dict__[key] = pandas.read_hdf(
                        saved_project, key=key, mode="r", data_columns=True
                    )
                else:
                    print("missing key:" + key)
            except Exception as e:
                if key == "par" and old_switch:
                    pass  # we have read it before already and the error is ok
        ...
        try:
            ...
            self.__dict__["re"]["fit_results_times"] = pardf_to_timedf(
                self.__dict__["re"]["fit_results_rates"]
            )
        except:
            pass
        ...
        for over_key in ["par_fit", "par"]:
            try:
                par_df = self.__dict__[over_key].loc[
        ...
        if fitted:
            ...
            if norm_window is not None:
                ref_scale = (
                    re["A"]
                    .loc[
                        norm_window[0] : norm_window[1], norm_window[2] : norm_window[3]
                    ]
                    .mean()
                    .mean()
                )
            ...
            lab = [
                "%g %s" % (ent, self.baseunit) + "_" + str(self.filename)
                for ent in rel_time
            ]
            han = handles[: len(rel_time) * 2]
            for ent in rel_time:
                lab.append(
                    "%g %s fit" % (ent, self.baseunit) + "_" + str(self.filename)
                )
            if other is not None:
                for i, o in enumerate(other):
                    ...
                    if norm_window is not None:
                        rel_scale = (
                            re["A"]
                            .loc[
                                norm_window[0] : norm_window[1],
                                norm_window[2] : norm_window[3],
                            ]
                            .mean()
                            .mean()
                        )
                        try:
                            ...
                            ax = plot_time(
                                ...
                                linewidth=linewidth,
                                scattercut=o.scattercut,
                                plot_second_as_energy=plot_second_as_energy,
                            )
                            handles, labels = ax.get_legend_handles_labels()
                            for ent in rel_time:
                                lab.append(
                                    "%g %s fit" % (ent, o.baseunit)
                                    + "_"
                                    + str(o.filename)
                                )
                    ...
                    if norm_failed:
                        ...
                        for ent in rel_time:
                            lab.append(
                                "%g %s fit" % (ent, o.baseunit) + "_" + str(o.filename)
                            )
                        for a in handles[-len(rel_time) :]:
                            han.append(a)
                        ...
                        if data_and_fit:
                            ...
                            for ent in rel_time:
                                lab.append(
                                    "%g %s" % (ent, o.baseunit) + "_" + str(o.filename)
                                )
                            for a in handles[-len(rel_time) :]:
                                han.append(a)
            if not norm_failed:
                ax.set_title(
                    "compare measured and fitted data at given times\n scaled to t=%g ps : %g ps , wl= %g nm: %g nm"
                    % (norm_window[0], norm_window[1], norm_window[2], norm_window[3])
                )
            else:
                    ...
                    if norm_window is not None:
                        ...
                        try:
                            ...
                            ax = plot_time(
                                ...
                                lines_are="data",
                                scattercut=o.scattercut,
                                bordercut=o.bordercut,
                                linewidth=linewidth,
                                wave_nm_bin=o.wave_nm_bin,
                                subplot=True,
                                color_offset=len(rel_time) * (i + 1),
                                plot_second_as_energy=plot_second_as_energy,
                            )
            ...
            if not scaling_failed:
                ax.set_title(
                    "compare measured and smoothed data at given times\n scaled to t=%g ps : %g ps , wl= %g nm: %g nm"
                    % (norm_window[0], norm_window[1], norm_window[2], norm_window[3])
                )
            else:
        ...
        if self.save_figures_to_folder:
            fig.savefig(
                check_folder(
                    ...
                    filename="compare_at_time_%s.png"
                    % "_".join(["%g" % a for a in rel_time]),
                ),
                bbox_inches="tight",
            )
    ...
    def Compare_at_wave(
        ...
        if fitted:
            ...
            if norm_window is not None:
                ...
                ax.set_title(
                    "compare measured and fitted data at given wavelength \n scaled to t=%g ps : %g ps , wl= %g nm: %g nm"
                    % (norm_window[0], norm_window[1], norm_window[2], norm_window[3])
                )
            else:
            ...
            if 1:
                ...
                if other is not None:
                    for i, o in enumerate(other):
                        ...
                        for ent in [
                            "%g nm" % a + "_" + str(o.filename) for a in rel_wave
                        ]:
                            lab.append(ent)
                        handles, labels = ax.get_legend_handles_labels()
                ...
                if norm_window is not None:
                    ax.set_title(
                        "compare measured and smoothed data at given wavelength \n scaled to t=%g ps : %g ps , wl= %g nm: %g nm"
                        % (
                            norm_window[0],
                            norm_window[1],
                            norm_window[2],
        ...
        if self.save_figures_to_folder:
            fig.savefig(
                check_folder(
                    path=self.figure_path,
                    filename="compare_at_wave_%s.png"
                    % "_".join(["%g" % a for a in rel_wave]),
                ),
                bbox_inches="tight",
            )
            return ax
    ...
    def Compare_DAC(self, other=None, spectra=None, separate_plots=False, cmap=None):
        ...
        if separate_plots:
            ...
            if self.scattercut is None:
                ...
                ax = DAC.loc[: self.scattercut[0], :].plot(
                    subplots=separate_plots,
                    figsize=(12, 10),
                    layout=(n_cols, 2),
                    legend=False,
                ...
                for i in range(len(scattercut) / 2 + 1):
                    if i == 0:
                        ...
                        for j, am in enumerate(ax):
                            DAC.loc[scattercut[2 * i - 1] : scattercut[2 * i], :].plot(
                                ax=a[j], legend=False, color=col, label="_nolegend_"
                            )
                    else:
                        for j, am in enumerate(ax):
                            DAC.loc[scattercut[-1] :, :].plot(
            ...
            if self.scattercut is None:
                ...
                ax = DAC.loc[self.scattercut[1] :, :].plot(
                    ax=ax,
                    subplots=separate_plots,
                    figsize=(16, 8),
                    legend=False,
                ...
                for i in range(len(scattercut) / 2 + 1):
                    if i == 0:
                        ...
                        ax = DAC.loc[scattercut[2 * i - 1] : scattercut[2 * i], :].plot(
                            ax=ax,
                            subplots=separate_plots,
                            figsize=(16, 8),
                            legend=False,
                ...
                if separate_plots:
                    ...
                    for j, am in enumerate(re["DAC"].columns):
                        if o.scattercut is None:
                            re["DAC"].iloc[:, j].plot(
                                subplots=False, ax=a[j], legend=False, color=col[i]
                            )
                            if j == 0:
                                handles, labels = a[0].get_legend_handles_labels()
                                hand.append(handles[-1])
                        elif isinstance(o.scattercut[0], numbers.Number):
                            re["DAC"].iloc[:, j].loc[: o.scattercut[0]].plot(
                                subplots=False, ax=a[j], legend=False, color=col[i]
                            )
                            if j == 0:
                                handles, labels = a[0].get_legend_handles_labels()
                                hand.append(handles[-1])
                            re["DAC"].iloc[:, j].loc[o.scattercut[1] :].plot(
                                subplots=False,
                                ax=a[j],
                                legend=False,
                                color=col[i],
                            ...
                            for m in range(len(scattercut) / 2 + 1):
                                if m == 0:
                                    re["DAC"].iloc[:, j].loc[: scattercut[0]].plot(
                                        subplots=False,
                                        ax=a[j],
                                        legend=False,
                                        color=col[i],
                                    ...
                                    re["DAC"].iloc[:, j].loc[
                                        scattercut[2 * m - 1] : scattercut[2 * m]
                                    ].plot(
                                        subplots=False,
                                        ax=a[j],
                                        legend=False,
                                    ...
                                    re["DAC"].iloc[:, j].loc[scattercut[-1] :].plot(
                                        subplots=False,
                                        ax=a[j],
                                        legend=False,
                                        color=col[i],
                    ...
                    if o.scattercut is None:
                        ax = DAC.plot(
                            ...
                            legend=False,
                            color=colors[
                                (i + 1) * len(species) : (i + 2) * len(species)
                            ],
                        )
                    elif isinstance(o.scattercut[0], numbers.Number):
                        ax = DAC.loc[: o.scattercut[0], :].plot(
                            subplots=separate_plots,
                            ax=ax,
                            legend=False,
                            color=colors[
                        ...
                        for i in range(len(scattercut) / 2 + 1):
                            if i == 0:
                                ...
                                ax = DAC.loc[
                                    scattercut[2 * i - 1] : scattercut[2 * i], :
                                ].plot(
                                    subplots=separate_plots,
                                    ax=ax,
                                    legend=False,
        ...
        if self.save_figures_to_folder:
            fig.savefig(
                check_folder(path=self.figure_path, filename="compare_DAC.png"),
                bbox_inches="tight",
            )
